...
class BaselineAgent:
    ...
    def update_dataframe(self, df):
        agent = BaselineAgent(df=df, conversation_session=self.session_id, llm=self.llm, file_name=self.file_name, user_id=self.user_id, history=self.chat_history)
        return agent
    ...
    def configure_user_msg_prompt(self):
        user_prompt = PromptTemplate(
            template=""""Please answer the my question: {input}""",
    ...
    def run(self, text, context=''):
        ...
        if self.model_name == "gpt-4-turbo":
            result = self.agent_with_trimmed_history.with_config(
                configurable={"llm": "gpt4", "session_id": self.session_id}).invoke({"input": text, "context": context})
        elif self.model_name == "gpt-4o-2024-08-06":
            result = self.agent_with_trimmed_history.with_config(
                configurable={"llm": "gpt4o", "session_id": self.session_id}).invoke({"input": text, "context": context})
        else:
            result = self.agent_with_trimmed_history.with_config(
                configurable={"llm": "gpt4omini", "session_id": self.session_id}).invoke({"input": text, "context": context})
